...
export default function useExplorationMode(lastRefreshTimeRef, loadingRef) {
  ...
  const handlePreferenceChangeFromStore = useStore(
    (state) => state.handlePreferenceChange,
  );
  const initializeExplorationMode = useStore(
    (state) => state.initializeExplorationMode,
  );
