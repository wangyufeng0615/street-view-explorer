...
- Atlas descriptions fetch the current Street View frame in parallel with reverse geocoding and send it to `OPENROUTER_SCENE_MODEL` (default `deepseek/deepseek-v4.1-flash`). A frame-fetch failure is visible and stops generation so Atlas cannot pretend to see a missing image. Text-only interest-region generation remains on `OPENROUTER_MODEL` / `AI_MODEL` (default `deepseek/deepseek-v4.1-flash`). Geo Guess uses `OPENROUTER_VISION_MODEL` (default `deepseek/deepseek-v4.1-flash`), disables model reasoning, and caps output at 480 tokens to stay within its 30-second request budget. `CN_AI_MODEL` is used only when no AI/shared proxy is configured.
...
- Description search uses `engine=exa, mode=fast`; the ordinary/deep evidence budgets remain 4/6 results and 3000/2500 characters per result. The legacy `deepseek/deepseek-v4-flash-vision-exp` model (including its 20260821 version) prefers Fireworks with provider fallback explicitly allowed; V4.1 Flash and unrelated model overrides use automatic routing. `OPENROUTER_DESCRIPTION_PROVIDER_SORT=off` restores Auto Exacto, while `latency`, `throughput`, or `price` explicitly selects a sort instead. `OPENROUTER_DESCRIPTION_SEARCH=auto` restores the previous search routing. Set both rollback values and recreate the backend to restore the old performance policy. See [the dated experiment](atlas-latency-2026-09-05.md); the samples do not establish a latency SLA.
