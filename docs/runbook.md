...
- Description search uses `engine=exa, mode=fast`; the ordinary/deep evidence budgets remain 4/6 results and 3000/2500 characters per result. The measured DeepSeek scene model (including its 20260821 version) prefers Fireworks with provider fallback explicitly allowed; unrelated model overrides retain automatic routing. `OPENROUTER_DESCRIPTION_PROVIDER_SORT=off` restores Auto Exacto, while `latency`, `throughput`, or `price` explicitly selects a sort instead. `OPENROUTER_DESCRIPTION_SEARCH=auto` restores the previous search routing. Set both rollback values and recreate the backend to restore the old performance policy. See [the dated experiment](atlas-latency-2026-09-05.md); the samples do not establish a latency SLA.
